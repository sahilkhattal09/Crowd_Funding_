...
function getUsers() {
    axios.get(`http://localhost/FinalYearProject/UpdateVerification.php?id=${id}`).then(function(rese) {
        ...
        setPname(rese.data.name);
        setTi(rese.data.gender);
        setDis(rese.data.dob);
    });
}
    return(
        <>
            ...
            <form onSubmit={handleSubmit( e => submitForm(e)) }style={{borderRadius:'20px',background:'#e9ecef',boxShadow:'0px 5px 10px 0px rgba(0, 0, 0, 0.3)'}}  encType="multipart/form-data">
              <div className="colums">
                <div className="item">
                  ...
                  <input id="name" type="text" value={nm} name="pname" placeholder='Enter name of the patient' {...register("pname", {required: true})} onChange={e =>handleChange(e)}/>  
                  <p style={{color:'red',fontSize:'13px'}}>{errors.pname?.type === "required" && "*Enter name"}</p>
                </div>
                <div className="item">
                  <label htmlFor="eaddress">Title<span>*</span></label>
                  <input id="title" type="text" value={ti} name="title"  placeholder='Enter Title'  {...register("title", {required: true})} onChange={e =>handleChange(e)}/>
